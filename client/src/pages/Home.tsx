...
import { ArrowRight, CarFront, Check, ChevronRight, CircleDollarSign, MapPin, Search as SearchIcon, ShieldCheck, SlidersHorizontal, Sparkles, Truck, Users, Zap } from "lucide-react";
...
export default function Home() {
  ...
  return <main>
    ...
    <section className="container shortcut-section"><div className="section-heading compact"><div><span className="eyebrow orange">NAVEGUE POR INTENÇÃO</span><h2>Encontre do seu jeito.</h2></div><Link href="/buscar" className="text-link">Ver todas <ArrowRight size={16} /></Link></div><div className="shortcut-grid">{[{ label: "Carros para APP", Icon: CarFront, query: "finalidade=APP" }, { label: "Carros elétricos", Icon: Zap, query: "categoria=Elétrico" }, { label: "Motos", Icon: CircleDollarSign, query: "categoria=Moto" }, { label: "Utilitários", Icon: Truck, query: "categoria=Utilitário" }, { label: "Planos mensais", Icon: ArrowRight, query: "periodo=mensal" }].map(({ label, Icon, query }) => <Link key={label} href={`/buscar?${query}`} className="shortcut-card"><span className="shortcut-icon"><Icon size={21} strokeWidth={1.9} aria-hidden="true" /></span><span>{label}</span><ChevronRight size={17} aria-hidden="true" /></Link>)}</div></section>
    <section className="inventory-section"><div className="container"><div className="section-heading"><div><span className="eyebrow orange">SELEÇÃO INICIAL · DEMONSTRAÇÃO</span><h2>Veículos prontos para rodar.</h2><p>Esta vitrine inicial apresenta exemplos do marketplace. Em breve, o Admin poderá aprovar anúncios reais para aparecerem em destaque.</p></div><Link href="/buscar" className="outline-button">Explorar todos <ArrowRight size={16} /></Link></div><div className="inventory-grid">{vehicles.slice(0, 3).map((vehicle) => <VehicleCard key={vehicle.id} vehicle={vehicle} />)}</div></div></section>
