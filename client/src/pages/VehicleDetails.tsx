...
import { ArrowLeft, ArrowRight, Check, ChevronLeft, ChevronRight, FileText, Heart, ImageOff, MapPin, MessageCircle, ShieldCheck, Wrench } from "lucide-react";
...
function mapPersistentVehicle(item: PersistentVehicle): Vehicle & { companyId: number; imageUrls: string[] } {
  ...
  return {
    ...
    image: item.images.find((image) => image.isCover)?.url ?? item.images[0]?.url ?? "",
...
export default function VehicleDetails() {
  ...
  const updateContactMutation = trpc.auth.updateContact.useMutation();
...
@AlugaRodas`;
  ...
  const gallery = useMemo(() => persistentVehicle?.imageUrls.length ? persistentVehicle.imageUrls : vehicle.image ? [vehicle.image] : [], [persistentVehicle, vehicle.image]);
  ...
  useEffect(() => {
    if (!leadName && user?.name) setLeadName(user.name);
    if (!leadPhone && user?.phone) setLeadPhone(formatBrazilianPhone(user.phone));
  }, [leadName, leadPhone, user?.name, user?.phone]);
  ...
  const openInterest = () => {
    if (!user) {
      navigate(`/entrar?redirect=${encodeURIComponent(`/veiculo/${vehicle.slug}`)}`);
      return;
    }
  ...
  const submitLead = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
        if (user?.role === "cliente" || user?.role === "user") {
          await updateContactMutation.mutateAsync({ phone: normalizedPhone });
